        ...
        o = i.WordArray = s.extend({
            ...
            concat: function (e) {
                ...
                if (e = e.sigBytes, this.clamp(), i % 4)
                    for (var n = 0; n < e; n++) t[i + n >>> 2] |= (r[n >>> 2] >>> 24 - n % 4 * 8 & 255) << 24 - (i + n) % 4 * 8;
        ...
        p = i.BufferedBlockAlgorithm = s.extend({
            ...
            _process: function (t) {
                var r = this._data,
                    ...
                    s = this.blockSize,
                    a = n / (4 * s);
                if (t = (a = t ? e.ceil(a) : e.max((0 | a) - this._minBufferSize, 0)) * s, n = e.min(4 * t, n), t) {
...
function (e) {
    ...
    for (var s = CryptoJS, o = (c = s.lib).WordArray, a = c.Hasher, c = s.algo, u = [], h = 0; 64 > h; h++) u[h] = 4294967296 * e.abs(e.sin(h + 1)) | 0;
    c = c.MD5 = a.extend({
        _doReset: function () {
...
function () {
    var e, t = CryptoJS,
        ...
        n = (e = t.algo).EvpKDF = r.extend({
            ...
            compute: function (e, t) {
                for (var r = (a = this.cfg).hasher.create(), n = i.create(), s = n.words, o = a.keySize, a = a.iterations; s.length < o;) {
                    c && r.update(c);
    ...
    t.EvpKDF = function (e, t, r) {
        ...
        }), c = c.CBC = h, h = (d.pad = {}).Pkcs7 = {
            pad: function (e, t) {
                for (var r, n = (r = (r = 4 * t) - e.sigBytes % r) << 24 | r << 16 | r << 8 | r, s = [], o = 0; o < r; o += 4) s.push(n);
        ...
        var p = t.CipherParams = r.extend({
            ...
            f = (c = (d.format = {}).OpenSSL = {
                stringify: function (e) {
                    var t = e.ciphertext;
                    return ((e = e.salt) ? i.create([1398893684, 1701076831]).concat(e).concat(t) : t).toString(s)
                },
                parse: function (e) {
            ...
            d = (d.kdf = {}).OpenSSL = {
                execute: function (e, t, r, n) {
                    return n || (n = i.random(8)), e = o.create({
                        keySize: t + r
            ...
            _ = t.PasswordBasedCipher = f.extend({
                ...
                encrypt: function (e, t, r, i) {
                    return r = (i = this.cfg.extend(i)).kdf.execute(r, e.keySize, e.ivSize), i.iv = r.iv, (e = f.encrypt.call(this, e, t, r.key, i)).mixIn(r), e
                },
                decrypt: function (e, t, r, i) {
                    return i = this.cfg.extend(i), t = this._parse(t, i.format), r = i.kdf.execute(r, e.keySize, e.ivSize, t.salt), i.iv = r.iv, f.decrypt.call(this, e, t, r.key, i)
                }
            })
    }(),
    function () {
        for (var e = CryptoJS, t = e.lib.BlockCipher, r = e.algo, i = [], n = [], s = [], o = [], a = [], c = [], u = [], h = [], p = [], f = [], d = [], _ = 0; 256 > _; _++) d[_] = 128 > _ ? _ << 1 : _ << 1 ^ 283;
        var l = 0,
            g = 0;
        for (_ = 0; 256 > _; _++) {
            ...
            s[l] = S << 24 | S >>> 8, o[l] = S << 16 | S >>> 16, a[l] = S << 8 | S >>> 24, c[l] = S, S = 16843009 * k ^ 65537 * v ^ 257 * m ^ 16843008 * l, u[y] = S << 24 | S >>> 8, h[y] = S << 16 | S >>> 16, p[y] = S << 8 | S >>> 24, f[y] = S, l ? (l = m ^ d[d[d[k ^ m]]], g ^= d[d[g]]) : l = g = 1
        }
        var w = [0, 1, 2, 4, 8, 16, 32, 64, 128, 27, 54];
        r = r.AES = t.extend({
            _doReset: function () {
                for (var e = (r = this._key).words, t = r.sigBytes / 4, r = 4 * ((this._nRounds = t + 6) + 1), n = this._keySchedule = [], s = 0; s < r; s++)
                    if (s < t) n[s] = e[s];
                    else {
                        var o = n[s - 1];
                        s % t ? 6 < t && 4 == s % t && (o = i[o >>> 24] << 24 | i[o >>> 16 & 255] << 16 | i[o >>> 8 & 255] << 8 | i[255 & o]) : (o = i[(o = o << 8 | o >>> 24) >>> 24] << 24 | i[o >>> 16 & 255] << 16 | i[o >>> 8 & 255] << 8 | i[255 & o], o ^= w[s / t | 0] << 24), n[s] = n[s - t] ^ o
                    } for (e = this._invKeySchedule = [], t = 0; t < r; t++) s = r - t, o = t % 4 ? n[s] : n[s - 4], e[t] = 4 > t || 4 >= s ? o : u[i[o >>> 24]] ^ h[i[o >>> 16 & 255]] ^ p[i[o >>> 8 & 255]] ^ f[i[255 & o]]
            },
            ...
            decryptBlock: function (e, t) {
                var r = e[t + 1];
                e[t + 1] = e[t + 3], e[t + 3] = r, this._doCryptBlock(e, t, this._invKeySchedule, u, h, p, f, n), r = e[t + 1], e[t + 1] = e[t + 3], e[t + 3] = r
            },
            _doCryptBlock: function (e, t, r, i, n, s, o, a) {
                for (var c = this._nRounds, u = e[t] ^ r[0], h = e[t + 1] ^ r[1], p = e[t + 2] ^ r[2], f = e[t + 3] ^ r[3], d = 4, _ = 1; _ < c; _++) {
                    ...
                    f = i[f >>> 24] ^ n[u >>> 16 & 255] ^ s[h >>> 8 & 255] ^ o[255 & p] ^ r[d++], u = l, h = g, p = y
                }
                l = (a[u >>> 24] << 24 | a[h >>> 16 & 255] << 16 | a[p >>> 8 & 255] << 8 | a[255 & f]) ^ r[d++], g = (a[h >>> 24] << 24 | a[p >>> 16 & 255] << 16 | a[f >>> 8 & 255] << 8 | a[255 & u]) ^ r[d++], y = (a[p >>> 24] << 24 | a[f >>> 16 & 255] << 16 | a[u >>> 8 & 255] << 8 | a[255 & h]) ^ r[d++], f = (a[f >>> 24] << 24 | a[u >>> 16 & 255] << 16 | a[h >>> 8 & 255] << 8 | a[255 & p]) ^ r[d++], e[t] = l, e[t + 1] = g, e[t + 2] = y, e[t + 3] = f
            },
            keySize: 8
        });
...
! function (e, t) {
    ...
    function r(e, t) {
        let r = md5("huanju@quickapp" + t).substring(16).toLowerCase(),
    ...
    function n(e) {
        ...
        if (function (e) {
            ...
            for (let i in e) {
                let n = e[i];
                if ("string" != typeof i && "number" != typeof i || !/^[0-9a-zA-Z_\u4e00-\u9fa5]{1,255}$/.test(i)) return !1;
    ...
    const _ = {
        ...
        l = new class {
            ...
            init(e) {
                ...
                r.package = o.package || s.packageName, r.versionName = o.versionName || s.versionName, r.minPlatformVersion = o.minPlatformVersion || "", r.name = o.name || s.name;
                try {
                ...
                Promise.all([u.deviceInfos(), u.getUserId(), u.netType(), u.has_shortcut()]).then(e => {
                    let t = Object.assign(r, ...e),
                        n = {};
                    c(p).then(e => {
                        e ? n.cuid = e : (n.cuid = function (e) {
                            let t = e || i();
                            return md5(Date.now() + "-" + i() + "-" + i() + "-" + i() + "-" + t)
                        }(t.userId || ""), a(p, n.cuid).then(e => {
                            e && (d.has_cuid_storage = !0)
                        })), this.state.cuid = n.cuid, n.req = this.state.req = function (e) {
                            let t = Date.now() + i() + i() + (e || i());
                            return md5(t)
                        }(n.cuid), this.state.data = { ..._.device(t, this.state.cuid),
                            ..._.public(n)
                        }
                    })
                })
            ...
            page_stat(e) {
                ...
                }), this.state.is_entry = 0, d.has_open_log = !0
            }
            page_end(e) {
                if (!this.state.cuid || !this.state.data) return;
                let t = Date.now();
                if (this.state.page) this.state.page.duration = t - this.state.page.sta_time, this.state.page.end_time = t, this.handle_submit();
                else {
                    let e = {
            ...
            err_report(r) {
                ...
                c(f).then(e => {
                    ...
                    }(r && r.day) == n && (o = Number(r.len) + 1), o <= 50 && function (e) {
                        return s.get({
                            url: "/e.gif?" + e
                        })
                    }(i).then(e => {
    ...
    const g = {
            ...
            track_event(e, r) {
                try {
                    ...
                    if (!n(i)) return void console.error('"event error":  please check track_event parameter. parameter should be "string" or "object"');
                    l.event_log({
                        ev_id: e,
